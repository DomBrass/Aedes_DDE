...
#A stage/phenotypically structured delay-differential equation model that predicts the effect of environmental 
#variation on the population dynamics of Aedes albopictus. Designed to be used in conjunction with RO2_public.jl which 
#provides the unputs used to produce the results shown in Figure 1 of the paper. .csvs describing life-history traits
#are outputs of models produced in Parametrisation.R and are also provided in the repository.
...
WL     = CSV.read(raw"WL_re.csv", header=true, DataFrame)      #Wing length as predicted by larval temperature and density
A_Long = CSV.read(raw"AMgam.csv", header=true, DataFrame)      #Adult longeivity as predicted by wing length and adult temperature
L_Surv = CSV.read(raw"Fin_LSurv.csv", header=true, DataFrame)  #Larval survival as predicted by larval temperature and density
L_Dur  = CSV.read(raw"LDgam.csv", header=true, DataFrame)      #Larval duration as predicted by larval temperature and density
...
sol = solve(prob,alg, reltol = 1e-3, abstol = 1e-3)  #Runs the model

#Processing of model outputs

out = hcat(sol.u...); #Total model output
...
Temp_sim = float(Temp.(sol.t)); #Vector of temperatures
...
#Computes observed oviposition activity
...
A = plot(sol, idxs = (numb_A_1:numb_A_2), label = nothing, color = :inferno, line_z = Wing_Vals', xlabel = "Time (Days)", ylabel = "Number of Adults")
